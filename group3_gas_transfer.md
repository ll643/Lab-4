...
1.	ProCoDA switches from “prepare to calibrate” to “calibrate” if accumulation pressure is less than or equal to minimum calibration pressure.
2.	ProCoDA switches from “calibrate” to “pause” when accumulator pressure is greater than maximum calibration pressure.
3.	If elapsed time in current state is greater than calibrate to aeration lag, then “pause” will go to the next state.
4.	To calculate maximum calibration pressure, first multiply max cal/source and source pressure, and add that to the pressure when the system is OFF. This is better than using a constant for the maximum calibration pressure because you don’t assume any constants for source pressure, so it is more accurate.
5.	To calculate predicted pressure when it is filled at a constant mass flow rate, use the ideal gas law (PV = nRT) and add the change in pressure to the initial pressure. In ProCoDA, it multiplys the air flow rate by R*T/V.
6.	Inputs to air valve control are air slope and air flow rate.
7.	Air valve control controls source pressure and accumulator pressure.
8.	Jonathan gave a thumbs up!
